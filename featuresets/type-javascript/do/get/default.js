...
import getJavascriptAST from "@/do/get/default/javascript:ast"
import getJavascriptDeps from "@/do/get/default/javascript:deps"
import getJavascriptDepmap from "@/do/get/default/javascript:depmap"
import getJavascriptDocs from "@/do/get/default/javascript:docs"
...
async function runJavaScriptModule({name, source, workspace, ast, deps, depmap}) {
    ...
    const createURL = (key, blobParts, type) => {
    ...
    const workspaceImplModuleURL = createURL(`${name}/workspace`, [workspaceImplModuleSource], 'text/javascript')
...
async function runJavascript(handlerInputs) {
    ...
    let source = await workspace.getAttribute({unit, name, attribute: 'javascript:source'})
    ...
    let deps = []
    try {
      deps = await workspace.getAttribute({unit, name, attribute: 'javascript:deps'})
    } catch (e) {
      console.warn('could not parse deps variables in source', e, {unit, name, source})
    }

    let depmap = {}
    try {
      depmap = await workspace.getAttribute({unit, name, attribute: 'javascript:depmap'})
    } catch (e) {
      console.warn('could not get depmap from source', e, {unit, name, source})
    }

    let ast, hasModuleSyntax
    try {
      ({ast, hasModuleSyntax} = await workspace.getAttribute({unit, name, attribute: 'javascript:ast'}))
    } catch (e) {
      console.warn('could not parse ast from source', e, {unit, name, source})
    }
...
async function runJavascriptScript({name, source, workspace, deps, depmap, ast}) {
    ...
    try {
        ...
        console.warn('could not parse free variables in source', e, {unit, name, source})
    }
